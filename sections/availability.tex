...
\paragraph{\studio{}}
The page is served as static files on the MOSAICS project site at
\url{https://www.cs.ox.ac.uk/mosaics/mosaics-studio/}. It needs a browser with
WebAssembly and Web Workers, and WebGL for the molecular viewer, about 25~MB on
first visit and a few hundred megabytes of memory during a run; the version
described here is 1.2.6. Chrome is the browser the work reported here was run
in. On 8 September 2026 the viewer failed to initialise under Safari on one
machine while the same page rendered under Chrome, so the browser matrix is
stated as tested rather than as supported. The source is at
